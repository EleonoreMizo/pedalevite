...
PluginAux &	Router::create_plugin_aux (Document &doc, PluginPool &plugin_pool, Document::PluginAuxList &aux_list, std::string model)
{
	...
	plug._pi_id = plugin_pool.create (
		plug._model, std::make_unique <PluginPoolHostMini> ()
	);
	assert (plug._pi_id >= 0);

	if (_sample_freq > 0)
	{
		PluginDetails &   details = plugin_pool.use_plugin (plug._pi_id);
		int         latency = 0;
		details.reset_plugin (_sample_freq, _max_block_size, latency);
	}
