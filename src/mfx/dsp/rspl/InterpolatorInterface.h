...
public:
	...
	inline int     process_block (float * const dest_ptr_arr [], const float * const src_ptr_arr [], int pos_dest, fstb::FixedPoint pos_src, int end_dest, int beg_src, int end_src, fstb::FixedPoint rate, fstb::FixedPoint rate_step);
	...
	static inline fstb::FixedPoint
	               integrate_rate (int len, const fstb::FixedPoint &rate, const fstb::FixedPoint &rate_step);
...
protected:
	...
	virtual int    do_get_impulse_len () const = 0;
	virtual fstb::FixedPoint
	               do_get_group_delay () const = 0;

	virtual void   do_start (int nbr_chn) = 0;
	virtual int    do_process_block (float * const dest_ptr_arr [], const float * const src_ptr_arr [], int pos_dest, fstb::FixedPoint pos_src, int end_dest, int beg_src, int end_src, fstb::FixedPoint rate, fstb::FixedPoint rate_step) = 0;
	virtual float  do_process_sample (const float src_ptr [], fstb::FixedPoint pos_src, fstb::FixedPoint rate) = 0;

	// Utility functions
	static bool    test_and_process_trivial_cases (int &nbr_spl_gen, int nbr_chn, float * const dest_ptr_arr [], const float * const src_ptr_arr [], int pos_dest, fstb::FixedPoint pos_src, int end_dest, int beg_src, int end_src, fstb::FixedPoint rate, fstb::FixedPoint rate_step, fstb::FixedPoint group_delay);
