...
bool	InterpolatorInterface::test_and_process_trivial_cases (long &nbr_spl_gen, int nbr_chn, float * const dest_ptr_arr [], const float * const src_ptr_arr [], long pos_dest, fstb::FixedPoint pos_src, long end_dest, long beg_src, long end_src, fstb::FixedPoint rate, fstb::FixedPoint rate_step, fstb::FixedPoint group_delay)
{
	    ...
	    && rate_step.get_val_int64 () == 0)
	{
		...
		{
			const long		nbr_spl_req = end_dest - pos_dest;
			const long		pos_src_int = pos_src.get_int_val ();
			const long		rate_int = rate.get_int_val ();

			// Copy
			if (rate_int == 1)
			{
				const long		nbr_available = end_src - pos_src_int;
			...
			else if (rate_int == 0)
			{
				...
				for (int chn = 0; chn < nbr_chn; ++chn)
				{
					...
					for (long pos = pos_dest; pos < end_dest; ++pos)
					{
						dest_ptr [pos] = val;
					}
				}
			...
			else
			{
				...
				const long		nbr_available_dest = nbr_available_src / rate_int;
				nbr_spl_gen = std::min (nbr_spl_req, nbr_available_dest);

				const long		pos_dest_stop = pos_dest + nbr_spl_gen;
				for (int chn = 0; chn < nbr_chn; ++chn)
				{
					const float *	src_ptr = src_ptr_arr [chn] + pos_src_int;
					float *			dest_ptr = dest_ptr_arr [chn];
					for (long pos = pos_dest; pos < pos_dest_stop; ++pos)
					{
