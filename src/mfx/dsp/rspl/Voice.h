...
public:
	...
	void				process_block (float *out_ptr_arr [], long nbr_spl);
...
private:
	...
	void				linearise_sample (long nbr_spl);
	long				collect_source_spl (long offset, long max_len, int cur_dir, int64_t pos, int64_t stop_pos);
	void				advance_cursor_pos (int64_t &pos, int &dir, long stride) const;
	void				check_finished (float *out_ptr_arr [], long &dest_pos, long nbr_spl);
	...
	InterpolatorInterface *			// 0: interpolator not set.
						_interp_ptr;
	long				_imp_len;		// Impulse length, samples. > 0
	...
	long				_tmp_buf_len;	// > 0
	long				_room_for_lin_src;	// Number of source samples. Valid only in active mode
	...
	long				_fade_rem_spl;	// 0 if no fade currently
