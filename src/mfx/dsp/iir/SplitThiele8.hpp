...
std::array <float, 2>	SplitThiele8::compute_ap4_lp4 (float x, SvfCore <> &flt_1, SvfCore <> &flt_2)
{
	const auto     s1     = flt_1.process_sample_mm (x);
	const float    lp2    = s1 [1];
	const float    bp2    = s1 [0];
	const float    hp2    = x - _r1 * bp2 - lp2;

	const auto     s2     = flt_2.process_sample_mm (lp2);
	const float    lp4    = s2 [1];
	const float    lp2bp2 = s2 [0];
	const float    lp2hp2 = lp2 - _r2 * lp2bp2 - lp4;

	const float    ap4    = hp2 + _m1 * bp2 + _m3 * lp2hp2 + _m4 * lp2bp2 + lp4;
