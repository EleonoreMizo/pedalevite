...
void	Simd <VD, VS>::scale_1_vlr (float data_ptr [], long nbr_spl, float s_vol, float e_vol)
{
	...
	vec_step = vec_step * fstb::ToolsSimd::load_f32 (_cst_4444);
...
void	Simd <VD, VS>::scale_2_vlr (float data_1_ptr [], float data_2_ptr [], long nbr_spl, float s_vol, float e_vol)
{
	...
	vec_vol += vec_step_spl;
	vec_step *= fstb::ToolsSimd::load_f32 (_cst_4444);
...
void	Simd <VD, VS>::copy_spread_1_2_vlr (float out_1_ptr [], float out_2_ptr [], const float in_ptr [], long nbr_spl, float s_vol_l, float s_vol_r, float e_vol_l, float e_vol_r)
{
	...
	const float		step_l = (e_vol_l - s_vol_l) / nbr_spl;
	const float		step_r = (e_vol_r - s_vol_r) / nbr_spl;
	...
	vec_step_l *= fstb::ToolsSimd::load_f32 (_cst_4444);
	...
	vec_step_r *= fstb::ToolsSimd::load_f32 (_cst_4444);
...
void	Simd <VD, VS>::copy_mat_2_2_v (float out_1_ptr [], float out_2_ptr [], const float in_1_ptr [], const float in_2_ptr [], long nbr_spl, const StereoLevel &vol)
{
	...
	const auto	vec_vol_l2l = fstb::ToolsSimd::set1_f32 (vol_l2l);
	const auto	vec_vol_r2l = fstb::ToolsSimd::set1_f32 (vol_r2l);
	const auto	vec_vol_l2r = fstb::ToolsSimd::set1_f32 (vol_l2r);
	const auto	vec_vol_r2r = fstb::ToolsSimd::set1_f32 (vol_r2r);
...
void	Simd <VD, VS>::copy_mat_2_2_vlr (float out_1_ptr [], float out_2_ptr [], const float in_1_ptr [], const float in_2_ptr [], long nbr_spl, const StereoLevel &s_vol, const StereoLevel &e_vol)
{
	...
	vec_step_l2l *= fstb::ToolsSimd::load_f32 (_cst_4444);
	...
	vec_step_r2l *= fstb::ToolsSimd::load_f32 (_cst_4444);
	...
	vec_step_l2r *= fstb::ToolsSimd::load_f32 (_cst_4444);
	...
	vec_step_r2r *= fstb::ToolsSimd::load_f32 (_cst_4444);
...
void	Simd <VD, VS>::copy_1_2i (float out_ptr [], const float in_ptr [], long nbr_spl)
{
	...
	{
		const auto	vec_data = V128Src::load_f32 (in_ptr);
		fstb::ToolsSimd::VectF32	vec_data_1;
		fstb::ToolsSimd::VectF32	vec_data_2;
...
void	Simd <VD, VS>::copy_2_4i2 (float out_ptr [], const float in_1_ptr [], const float in_2_ptr [], long nbr_spl)
{
	...
	for (long nbr_loop = nbr_spl >> 2; nbr_loop > 0; --nbr_loop)
	{
		...
		// First unpacking
		fstb::ToolsSimd::VectF32   o1;	// A1 A2 B1 B2
		fstb::ToolsSimd::VectF32   o3;	// C1 C2 D1 D2
...
template <class VD, class VS>
const typename Simd <VD, VS>::VectFloat4Aligned	Simd <VD, VS>::_cst_0123 =
{
	0.0f, 1.0f, 2.0f, 3.0f
};

template <class VD, class VS>
const typename Simd <VD, VS>::VectFloat4Aligned	Simd <VD, VS>::_cst_4444 =
{
	4.0f, 4.0f, 4.0f, 4.0f
};
