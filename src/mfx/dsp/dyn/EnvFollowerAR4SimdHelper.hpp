...
fstb::Vf32	EnvFollowerAR4SimdHelper <VD, VS, VP, ORD>::process_sample (const fstb::Vf32 &in) noexcept
{
	...
	for (int flt = 0; flt < ORD; ++flt)
	{
		...
		const auto     coef       = fstb::select (delta_lt_0, coef_r, coef_a);
...
// Can work in-place.
template <class VD, class VS, class VP, int ORD>
void	EnvFollowerAR4SimdHelper <VD, VS, VP, ORD>::process_block (fstb::Vf32 out_ptr [], const fstb::Vf32 in_ptr [], int nbr_spl) noexcept
{
...
// Input data must contain only positive values!
template <class VD, class VS, class VP, int ORD>
fstb::Vf32	EnvFollowerAR4SimdHelper <VD, VS, VP, ORD>::analyse_block (const fstb::Vf32 in_ptr [], int nbr_spl) noexcept
{
	assert (V128Src::check_ptr (in_ptr));
	assert (nbr_spl > 0);
