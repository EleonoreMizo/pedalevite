...
public:
	...
	int            get_prog_index () const;
	const OverrideMap &
	               use_param_ctrl_override_map () const;

	std::set <std::string>
	               collect_labels (bool cur_prog_flag) const;
	...
	static void    update_parameter (doc::Program &prog, int slot_index, PiType type, int index, float val);
	static float   get_param_val (const doc::Program &prog, int slot_id, PiType type, int index);
...
private:
	...
	void           collect_labels (std::set <std::string> &labels, const doc::Program &prog) const;
