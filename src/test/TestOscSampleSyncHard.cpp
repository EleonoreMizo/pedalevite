...
int	TestOscSampleSyncHard::perform_test ()
{
	...
	// !!! Depends on CalcDataType. See the oscillator code for details.
	const int		scale = 0x4000;
	osc::WpdGen::build_blep_pure (steptable, win_gen, float (scale));

#if 0
	std::vector <float> blep_data (Steptable::NBR_PHASES * Steptable::PHASE_LEN);
	for (int pos = 0; pos < int (blep_data.size ()); ++pos)
	{
	...
	osc.set_steptable (steptable);
	osc.set_bandlimit (-0x10000);
	osc.set_base_pitch (16 * 0x10000);

	printf ("Testing OscSampleSyncHard...\n");
...
void	TestOscSampleSyncHard::perform_test_internal (OscType &osc, std::vector <float> &result_m, int dest_len, int block_len, int sync_pos_start, float sync_pos_speed, int master_pitch_start, int master_pitch_end, int slave_pitch_start, int slave_pitch_end)
{
...
void	TestOscSampleSyncHard::perform_test_internal_2 (OscType &osc, std::vector <OscType::CalcDataType> &dest, int dest_len, int block_len, int sync_pos_start, float sync_pos_speed, int master_pitch_start, int master_pitch_end, int slave_pitch_start, int slave_pitch_end)
{
	assert (dest_len > 0);
	assert (block_len > 0);
	...
	for (int block_pos = 0; block_pos < dest_len; block_pos += block_len)
	{
		...
		int            wave_length  = osc.get_wave_length ();
...
void	TestOscSampleSyncHard::add_to_dest (std::vector <float> &result_m, const std::vector <OscType::CalcDataType> &dest_int)
{
	// Format conversion
