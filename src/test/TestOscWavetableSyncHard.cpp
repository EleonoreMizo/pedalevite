/*****************************************************************************

        TestOscWavetableSyncHard.cpp
        Author: Laurent de Soras, 2019

--- Legal stuff ---

This program is free software. It comes without any warranty, to
the extent permitted by applicable law. You can redistribute it
and/or modify it under the terms of the Do What The Fuck You Want
To Public License, Version 2, as published by Sam Hocevar. See
http://sam.zoy.org/wtfpl/COPYING for more details.

*Tab=3***********************************************************************/



#if defined (_MSC_VER)
	#pragma warning (1 : 4130 4223 4705 4706)
	#pragma warning (4 : 4355 4786 4800)
#endif



/*\\\ INCLUDE FILES \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*/

#include "fstb/lang/type_name.h"
#include "fstb/def.h"
#include "fstb/fnc.h"
#include "mfx/dsp/osc/OscWavetable.h"
#include "mfx/dsp/osc/OscWavetableSyncHard.h"
#include "mfx/dsp/osc/WavetableMipMapper.h"
#include "mfx/dsp/osc/WpdGen.h"
#include "mfx/dsp/rspl/InterpFtor.h"
#include "mfx/dsp/wnd/Generic.h"
#include "mfx/dsp/wnd/CoefGenBHMinLobe.h"
#include "mfx/FileOpWav.h"
#include "test/TestOscWavetableSyncHard.h"
#include "test/TimerAccurate.h"

#include <limits>
#include <typeinfo>
#include <vector>

#include <cassert>
#include <cstdint>
#include <cstdio>



/*\\\ PUBLIC \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*/



int	TestOscWavetableSyncHard::perform_test ()
{
	int            ret_val = 0;

	typedef mfx::dsp::rspl::InterpFtor::CubicHermite Interpolator;

	typedef mfx::dsp::osc::OscWavetable <
		Interpolator,
		12, 6, 3,
		float,
		Interpolator::DATA_PRE, Interpolator::DATA_POST
	> OscInternalFloat;
	typedef mfx::dsp::osc::OscWavetableSyncHard <
		OscInternalFloat, 16, 8, 0
	> OscTypeFloat;

	if (ret_val == 0)
	{
		ret_val = test_valid <OscTypeFloat> ();
	}

	if (ret_val == 0)
	{
		test_speed <OscTypeFloat> ();
	}

	typedef mfx::dsp::osc::OscWavetable <
		Interpolator,
		12, 6, 3,
		int16_t,
		Interpolator::DATA_PRE, Interpolator::DATA_POST
	> OscInternalInt16;
	typedef mfx::dsp::osc::OscWavetableSyncHard <
		OscInternalInt16, 16, 8, 14
	> OscTypeInt16;

	if (ret_val == 0)
	{
		ret_val = test_valid <OscTypeInt16> ();
	}

	if (ret_val == 0)
	{
		test_speed <OscTypeInt16> ();
	}

	return (ret_val);
}



/*\\\ PROTECTED \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*/



/*\\\ PRIVATE \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*/



template <typename O>
int	TestOscWavetableSyncHard::test_valid ()
{
	typedef O OscType;
	typedef typename OscType::DataType DataType;
	typedef typename OscType::WavetableDataType WtType;
	typedef typename OscType::AntialiasedStep StType;

	printf (
		"Testing %s...\n",
		fstb::lang::type_name <OscType> ().to_str ().c_str ()
	);

	std::vector <float>  result_m;

	OscType        osc;
	WtType         wt;
	StType         st;
	configure_osc (osc, wt, st);

	int            wt_len_spl = 0;
	for (int t_idx = 0; t_idx < wt.get_nbr_tables (); ++t_idx)
	{
		wt_len_spl += wt.get_table_len (t_idx);
		wt_len_spl += WtType::UNROLL_PRE;
		wt_len_spl += WtType::UNROLL_POST;
	}
	const int      wt_len_bytes = wt_len_spl * sizeof (DataType);
	const int      st_len_bytes = StType::TABLE_SIZE * sizeof (DataType);
	printf ("Total wavetable size: %d bytes.\n", wt_len_bytes);
	printf ("Total steptable size: %d bytes.\n", st_len_bytes);

	const int      base_pitch = osc.get_base_pitch ();

	const int      sample_freq = 44100;
	const int      block_len   = 64;
	const int      len         = (sample_freq * 16) & ~(block_len - 1);
	const int      len_s       = (sample_freq *  4) & ~(block_len - 1);
	assert (len   % block_len == 0);
	assert (len_s % block_len == 0);
	std::vector <DataType>  data (len);

	const int32_t  pitch_1000hz =
		osc.conv_freq_to_pitch (1000.f, float (sample_freq));
	const int32_t  pitch_262hz  =
		osc.conv_freq_to_pitch ( 261.6255653f, float (sample_freq));
	const int32_t  pitch_100hz  =
		osc.conv_freq_to_pitch ( 100.f, float (sample_freq));
	const int32_t  pitch_262hz_16  =
		osc.conv_freq_to_pitch ( 261.6255653f*16, float (sample_freq));
	const int32_t  pitch_100hz_16  =
		osc.conv_freq_to_pitch ( 100.f*16, float (sample_freq));

	test_sweep (
		osc, result_m, len, block_len,
		base_pitch - (10 << OscType::PITCH_FRAC_BITS), base_pitch,
		pitch_100hz, pitch_100hz,
		0
	);
	test_sweep (
		osc, result_m, len, 0,
		base_pitch - (10 << OscType::PITCH_FRAC_BITS), base_pitch,
		pitch_100hz, pitch_100hz,
		0
	);

	test_sweep (
		osc, result_m, len, block_len,
		pitch_100hz, pitch_100hz,
		base_pitch - (10 << OscType::PITCH_FRAC_BITS), base_pitch,
		0
	);
	test_sweep (
		osc, result_m, len, 0,
		pitch_100hz, pitch_100hz,
		base_pitch - (10 << OscType::PITCH_FRAC_BITS), base_pitch,
		0
	);

	// Static, low pitch
	test_sweep (
		osc, result_m, len_s, block_len,
		pitch_100hz, pitch_100hz,
		pitch_262hz, pitch_262hz,
		0
	);
	test_sweep (
		osc, result_m, len_s, block_len,
		pitch_262hz, pitch_262hz,
		pitch_100hz, pitch_100hz,
		0
	);

	// Static, high pitch
	test_sweep (
		osc, result_m, len_s, block_len,
		pitch_100hz_16, pitch_100hz_16,
		pitch_262hz_16, pitch_262hz_16,
		0
	);
	test_sweep (
		osc, result_m, len_s, block_len,
		pitch_262hz_16, pitch_262hz_16,
		pitch_100hz_16, pitch_100hz_16,
		0
	);

	osc.clear_buffers ();
	osc.set_sync_pos (0);

	osc.set_pitch_slave (pitch_1000hz);
	for (int pos = 0; pos < len_s; pos += block_len)
	{
		const double	freq  =
			1000 * exp (cos (pos * 2 * fstb::PI * 3 / sample_freq) * 1);
		const int32_t  pitch =
			osc.conv_freq_to_pitch (float (freq), float (sample_freq));
		osc.set_pitch (pitch);
		osc.process_block (&data [pos], block_len);
	}

	add_result <O> (result_m, data, len_s);

	osc.set_pitch (pitch_100hz);
	for (int pos = 0; pos < len_s; pos += block_len)
	{
		const double	freq  =
			1000 * exp (cos (pos * 2 * fstb::PI * 3 / sample_freq) * 1);
		const int32_t  pitch =
			osc.conv_freq_to_pitch (float (freq), float (sample_freq));
		osc.set_pitch_slave (pitch);
		osc.process_block (&data [pos], block_len);
	}

	add_result <O> (result_m, data, len_s);

	std::string filename = "results/oscwavetablesynchard";
	filename += typeid (DataType).name ();
	filename += "0.wav";
	mfx::FileOpWav::save (filename.c_str (), result_m, 44100, 0.5f);

	printf ("Done.\n");

	return 0;
}



template <typename O, typename V>
void	TestOscWavetableSyncHard::test_sweep (O &osc, V &result_m, int len, int block_len, int32_t pitch_beg, int32_t pitch_end, int32_t pitch_slave_beg, int32_t pitch_slave_end, uint32_t sync_pos)
{
	std::vector <typename O::DataType>  data (len);

	osc.clear_buffers ();
	osc.set_sync_pos (sync_pos);

	const int64_t  dif_pitch       =
		int64_t (pitch_end) - int64_t (pitch_beg);
	const int64_t  dif_pitch_slave =
		int64_t (pitch_slave_end) - int64_t (pitch_slave_beg);

	if (block_len <= 0)
	{
		for (int pos = 0; pos < len; ++pos)
		{
			const int32_t  pitch       =
				pitch_beg + int32_t (pos * dif_pitch / len);
			const int32_t  pitch_slave =
				pitch_slave_beg + int32_t (pos * dif_pitch_slave / len);
			osc.set_pitch (pitch);
			osc.set_pitch_slave (pitch_slave);
			data [pos] = osc.process_sample ();
		}
	}

	else
	{
		for (int pos = 0; pos < len; pos += block_len)
		{
			const int32_t  pitch       =
				pitch_beg + int32_t (pos * dif_pitch / len);
			const int32_t  pitch_slave =
				pitch_slave_beg + int32_t (pos * dif_pitch_slave / len);
			osc.set_pitch (pitch);
			osc.set_pitch_slave (pitch_slave);
			osc.process_block (&data [pos], block_len);
		}
	}

	add_result <O> (result_m, data, len);
}



template <typename O>
void	TestOscWavetableSyncHard::test_speed ()
{
	typedef O OscType;
	typedef typename OscType::DataType DataType;

	const int      block_len  = 256;
	const int      nbr_blocks = 65536;

	OscType        osc;
	typename OscType::WavetableDataType wt;
	typename OscType::AntialiasedStep   st;
	configure_osc (osc, wt, st);

	const int32_t  pitch_1000hz = osc.conv_freq_to_pitch (1000, 44100);
	osc.set_pitch (pitch_1000hz);
	osc.set_pitch_slave (pitch_1000hz);

	std::vector <DataType>   dest (block_len);
	DataType *     dest_ptr = &dest [0];

	printf (
		"Speed test for %s...\n",
		fstb::lang::type_name <OscType> ().to_str ().c_str ()
	);

	TimerAccurate  tim;
	double         acc_dummy = 0;

	tim.reset ();
	tim.start ();
	for (int block_cnt = 0; block_cnt < nbr_blocks; ++block_cnt)
	{
		osc.process_block (dest_ptr, block_len);

		// Prevents the optimizer to remove all the loops
		acc_dummy += double (dest_ptr [block_len - 1]);
	}
	tim.stop ();

	double	      spl_per_s = tim.get_best_rate (block_len * nbr_blocks);
	spl_per_s += fstb::limit (acc_dummy, -1e-300, 1e-300); // Anti-optimizer trick
	const double   mega_sps  = spl_per_s / 1000000.0;
	printf ("Speed: %12.3f Mspl/s.\n\n", mega_sps);
}



template <typename O>
void	TestOscWavetableSyncHard::configure_osc (O &osc, typename O::WavetableDataType &wt, typename O::AntialiasedStep &st)
{
	using namespace mfx::dsp;

	typedef O OscType;
	typedef typename OscType::DataType DataType;

	// Antialiased step generation
	wnd::Generic <double, wnd::CoefGenBHMinLobe> win_gen;
	osc::WpdGen::build_blep_pure (st, win_gen, 1 << OscType::PHASE_LEVEL_L2);

	// Wavetable generation
	const float    scale      = get_data_scale <OscType> ();
	const int      last_table = wt.get_nbr_tables () - 1;
	const int      table_len  = wt.get_table_len (last_table);
	for (long pos = 0; pos < table_len; ++pos)
	{

#if 1
		// Saw
		wt.set_sample (
			last_table,
			(pos - table_len / 4) & (table_len - 1),
			DataType (((float (pos * 2 + 1) / float (table_len)) - 1) * scale)
		);
#else
		// Sine
		wt.set_sample (
			last_table,
			pos,
			DataType (cos (pos * (2 * basic::PI) / double (table_len)) * scale)
		);
#endif
	}

	// Mipmapping
	typedef typename OscType::WavetableDataType  WavetableDataType;
	osc::WavetableMipMapper <WavetableDataType>  mipmapper;
	mipmapper.build_mipmaps (wt);

	osc.set_wavetable (wt);
	osc.set_steptable (st);
	osc.set_base_pitch (16 << OscType::PITCH_FRAC_BITS);
}



template <typename T, bool INT_FLAG>
class TestOscWavetableSyncHard_GetNumScale
{
public:
	static float   get () { return 1.f; }
};

template <typename T>
class TestOscWavetableSyncHard_GetNumScale <T, true>
{
public:
	static float   get ()
	{
		return float (T (1) << (std::numeric_limits <T>::digits - 1));
	}
};

template <typename O>
float	TestOscWavetableSyncHard::get_data_scale ()
{
	typedef O OscType;
	typedef typename OscType::DataType DataType;

	return TestOscWavetableSyncHard_GetNumScale <
		DataType,
		std::numeric_limits <DataType>::is_integer
	>::get ();
}



template <typename O>
void	TestOscWavetableSyncHard::add_result (std::vector <float> &result_m, const std::vector <typename O::DataType> &data, size_t len)
{
	if (len == 0)
	{
		len = data.size ();
	}
	assert (len <= data.size ());
	const float    scale = get_data_scale <O> ();
	const float    scale_inv = 1.0f / scale;
	std::for_each (
		data.begin (),
		data.begin () + len,
		[&] (const typename O::DataType val)
		{
			result_m.push_back (float (val) * scale_inv);
		}
	);
}



/*\\\ EOF \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*/
